...
export const UIProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [theme, setTheme] = useState<Theme>(() => {
    const saved = localStorage.getItem('ruldatest_theme');
    return (saved as Theme) || 'light';
  });

  const [language, setLanguage] = useState<Language>(() => {
    const saved = localStorage.getItem('ruldatest_lang');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('ruldatest_theme', theme);
  }, [theme]);

  useEffect(() => {
    localStorage.setItem('ruldatest_lang', language);
  }, [language]);
