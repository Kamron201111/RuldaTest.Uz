    ...
    <div className="max-w-2xl mx-auto p-4 h-[calc(100vh-80px)] flex flex-col">
      <div className="flex items-center gap-4 mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
         ...
         <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-full bg-sky-100 dark:bg-sky-900 flex items-center justify-center text-sky-600 dark:text-sky-400 overflow-hidden">
                {targetUser.avatar ? (
            ...
            messages.map(m => {
               ...
               return (
                 <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-2xl ${isMe ? 'bg-sky-500 text-white rounded-tr-none' : 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-600 rounded-tl-none'}`}>
                       <p className="text-sm">{m.text}</p>
                       <div className="flex items-center justify-end gap-1 mt-1">
                           <p className={`text-[10px] ${isMe ? 'text-sky-200' : 'text-slate-400'}`}>
      ...
      <div className="flex gap-2">
         <input
            ...
            className="flex-1 p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-sky-400 outline-none text-slate-800 dark:text-white"
