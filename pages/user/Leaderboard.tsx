      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700">
          <button 
            ...
            className={`flex-1 py-4 font-medium text-sm transition-colors ${period === 'daily' ? 'text-sky-600 border-b-2 border-sky-500 bg-sky-50 dark:bg-sky-900/20' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
          >
            {t('lead_daily')}
          </button>
          <button 
             onClick={() => setPeriod('monthly')}
             className={`flex-1 py-4 font-medium text-sm transition-colors ${period === 'monthly' ? 'text-sky-600 border-b-2 border-sky-500 bg-sky-50 dark:bg-sky-900/20' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
          >
            {t('lead_monthly')}
          </button>
          <button 
             onClick={() => setPeriod('yearly')}
             className={`flex-1 py-4 font-medium text-sm transition-colors ${period === 'yearly' ? 'text-sky-600 border-b-2 border-sky-500 bg-sky-50 dark:bg-sky-900/20' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
          >
            {t('lead_yearly')}
          </button>
        </div>
